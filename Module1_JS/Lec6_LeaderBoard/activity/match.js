...
function parseData(html) {
    // It is a html of a single match
    ...
    for(let i = 0; i < bothInnings.length; i++) {
        ...
        fs.writeFileSync("leaderboard.json" , JSON.stringify(object));
        //fs.writeFileSync("leaderboard.txt", object);
        //console.log(object);
        console.log("#################################################################");
    }
}
...
function processLeaderboard(teamName , batsmanName , runs , balls , fours , sixes ){
    ...
    for(let i = 0; i < object.length; i++) {
        let iIndexObjext = object[i];
        let val = iIndexObjext[batsmanName];
        //console.log("Value is: " + val);
        if(val != undefined) {
            iIndexObjext[runs] += runs;
            iIndexObjext[balls] += balls;
            iIndexObjext[fours] += fours;
            iIndexObjext[sixes] += sixes;
            flag = true;
            return;
        }
    }
